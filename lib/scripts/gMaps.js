...
const flBeaches = { lat: 26.92, lng: -81.22  };
...
function anotherControl(controlDiv) {
   ...
   controlText.innerHTML = `Click on icons to Auto-Zoom`;
   controlText.style.fontFamily = "Josefin Sans, monotype";
   controlUI.appendChild(controlText);
}
...
function initMap() {
   ...
   const centerControlDiv5 = document.createElement("div");
   CenterControl(centerControlDiv5, map, "Florida Beaches", flBeaches, 7);
   ...
   map.controls[google.maps.ControlPosition.LEFT_TOP].push(centerControlDiv5);
   ...
   locations.forEach(loc => {
      ...
      markers.push(rentMarker);
   });

   if (window.location.href.includes('github.io/sandhelper/locations') || window.location.href.includes('file://')) {
...
theStates.forEach(s => {
   ...
   const shouldAlphabetize = (s === "Florida");
   const theBeachesForStateS = shouldAlphabetize
      ? theBeachesKeyedByState[s].sort((a, b) => (a.beach > b.beach) ? 1 : -1)
      : theBeachesKeyedByState[s];
   theBeachesForStateS.forEach(bb => {
      ...
      rentalAgentsKeyedByBeach[bb.beach].forEach(ra => {
         ...
         thePhoneNumberLink.classList.add("hover-underline");
         ...
         theEmailLink.classList.add("hover-underline");
...
const plusMinusButtons = document.querySelectorAll(
   "#locations-accordion-container > div > ul > li > div > div > button"
);
